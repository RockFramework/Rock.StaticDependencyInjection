...
namespace Rock.StaticDependencyInjection.AcceptanceTests
{
    public class ImportMultipleTests
    {
        ...
        {
            var registeredInstances = ((IEnumerable)ServiceLocator.Get(serviceAbstractionType, serviceName)).Cast<object>().ToList();
        ...
        public void GivenAMultipleImplementationForTheAbstraction_ThenTheFirstAlphabeticalImplementationIsUsed(Type serviceAbstractionType, Type[] expectedServiceTypes, string serviceName)
        {
            var registeredInstances = ((IEnumerable)ServiceLocator.Get(serviceAbstractionType, serviceName)).Cast<object>().ToList();
            Assert.That(registeredInstances.Count(), Is.EqualTo(expectedServiceTypes.Length));
